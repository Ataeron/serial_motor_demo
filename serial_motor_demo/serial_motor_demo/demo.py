...
from serial_motor_demo_msgs.msg import RangeVals
...
class MotorDriver(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('serial_port_one', value="/dev/ttyUSB0")
        self.serial_port_one = self.get_parameter('serial_port_one').value

        self.declare_parameter('serial_port_two', value="/dev/ttyUSB0") # Change value to port for 4wd platform
        self.serial_port_two = self.get_parameter('serial_port_two').value


        self.declare_parameter('baud_rate', value=115200) # baud rate to match ToF sensor
        ...
        self.subscription = self.create_subscription(
            ...
            10)
        self.range_pub = self.create_publisher(RangeVals, 'range_vels', 10)
        ...
        self.last_range_read_time = time.time()
        # self.last_m1_enc = 0
        # self.last_m2_enc = 0
        self.m1_spd = 0.0
        self.m2_spd = 0.0
        ...
        print(f"Connecting to port {self.serial_port_one} at {self.baud_rate}.")
        self.conn = serial.Serial(self.serial_port_one, self.baud_rate, timeout=1.0)
    ...
    def send_pwm_motor_command(self, mot_1, mot_2):
        self.send_command(f"o {int(mot_1)} {int(mot_2)}")
    ...
    def send_range_read_command(self):
    ...
    def check_range(self):
        resp = self.send_range_read_command()
        if (resp):

            new_time = time.time()
            time_diff = new_time - self.last_range_read_time
            self.last_range_read_time = new_time
            ...
            spd_msg = MotorVels()
            spd_msg.mot_1_perc = self.m1_spd
            spd_msg.mot_2_perc = self.m2_spd
